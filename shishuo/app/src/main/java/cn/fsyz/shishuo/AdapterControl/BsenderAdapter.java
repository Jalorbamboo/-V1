...
//每个话题的投稿列表适配器
public class BsenderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    Context mContext;
    private List<Bsender> dataList = new ArrayList<>();
    ...
    public void addAllData(List<Bsender> dataList) {
    ...
    public BsenderAdapter(Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType==TYPE_IMAGEVIEW){
            ...
            return new BsenderAdapter.ViewViewHolder(view2);
            ...
            return new BsenderAdapter.TextViewHolder(v);
        }
        return null;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        Bsender feedback = dataList.get(position);
        ...
        if (holder instanceof BsenderAdapter.ViewViewHolder) {
            //设置标题
            ((BsenderAdapter.ViewViewHolder) holder).title.setText(feedback.getTitle());
            //设置内容
            ((BsenderAdapter.ViewViewHolder) holder).content.setText(feedback.getContent());
            ((BsenderAdapter.ViewViewHolder) holder).url.setText(feedback.getPhoto());
            ((BsenderAdapter.ViewViewHolder) holder).id.setText(feedback.getObjectId());
            ((BsenderAdapter.ViewViewHolder) holder).time.setText(feedback.getCreatedAt());
            Picasso.with(mContext).load(feedback.getPhoto()).config(Bitmap.Config.RGB_565)
                    .resize(dp2px(800), dp2px(250)).centerCrop().into(((BsenderAdapter.ViewViewHolder) holder).cover);
        }else if (holder instanceof BsenderAdapter.TextViewHolder){
            ((BsenderAdapter.TextViewHolder) holder).id1.setText(feedback.getObjectId());
            ((BsenderAdapter.TextViewHolder) holder).content1.setText(feedback.getContent());
            ((BsenderAdapter.TextViewHolder) holder).title1.setText(feedback.getTitle());
            ((BsenderAdapter.TextViewHolder) holder).time1.setText(feedback.getCreatedAt());
        }
    ...
    public void setOnItemClickListener(BsenderAdapter.OnItemClickListener listener) {
